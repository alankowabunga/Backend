...
public class GamingConfig {
    ...
    //create Beans of GameRunner Interface to use run() method
    ...
    //Interface's Bean: 配置中，gameRunner 方法中的參數 GamingConsole game 會由 Spring 自動注入。因為在 Spring 容器中已經有一個 GamingConsole 類型的 Bean，即 pacmanGame，所以 Spring 會將 PacmanGame 的實例注入到 gameRunner Bean 中。
    @Bean
